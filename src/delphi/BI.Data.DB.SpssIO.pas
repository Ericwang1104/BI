...
type
  ...
  TspssMultRespDef = record
    {Nome p/ multi resposta terminado em null, tamanho m醲imo de 64 inicia "$" }
    ...
    {1 se multi resposta dictioma e 0 se n鉶                                   }
    qIsDichotomy       : integer;

    {1 se cont閙 valor numerico e 0 se n鉶                                     }
    qIsNumeric         : integer;

    {1 se v醨ias categorias para as quais o dicotomias est鉶 a ser marcados com}
    {o valor correspondente r髏ulos ao valor contabilizado.                    }
    qUseCategoryLabels : integer;

    {1 se para m鷏tiplas dicotomias para a qual o r髏ulo para o conjunto ?    }
    {retirado do r髏ulo da vari醰el primeiro componente vari醰el               }
    qUseFirstVarLabel  : integer;

    {Reservado para expans鉶}
    Reserved           : array[0..13] of integer;

    {O valor contabilizado para num閞ico m鷏tiplos dicotomias                  }
    nCountedValue      : longint;

    {Ponteiro o valor contabilizado para string de m鷏tiplos dicotomias        }
    pszCountedValue    : PAnsiChar;

    {Ponteiro para o array de nomes}
    ppszVarNames       : PTValuec;

    {Numero de vari醰eis da estrutura}
    nVariables          : integer;
  end;

  {Manipula玢o de arquivos}

  {Esta fun玢o abre o aruqivo para leitura, retornando handle para opera珲es no}
  {arquivo                                                                     }
  ...
  {Esta fun玢o retorna o valor do label apartir do nome curto da vari醰el}
  ...
  {Esta fun玢o grava um case no o arquivo de dados especificado                }
  {Ela deve ser chamada ap髎 ter sido setado os valores das vari醰eis, atrav閟 }
  ...
  {Esta fun玢o grava o dicion醨io de dados no arquivo associado                }
  {Antes da escrita do case o dicion醨io de dados deve esta comitado           }
  {handle       - Handle do arquivo.                                           }
  TspssCommitHeader          = function(const handle : integer) : integer; {$IFDEF WIN32} stdcall; {$ENDIF}

  {Manipula玢o de data e hora}
  {28/04/2008}

  {Esta fun玢o converte data para o formato gregoriano                         }
  {day        - Dias do m阺                                                    }
  {month      - meses do ano                                                   }
  ...
  {Esta fun玢o converte a data a partir de um valor interno para gregoriano    }
  {day        - Dias do m阺                                                    }
  {month      - meses do ano                                                   }
  {year       - ano a partir de 1582                                           }
  {spssdate   - Data no formato gregoriano                                     }
  ...
  {Esta fun玢o quebra um valor no formato interno SPSS(data) em n鷐ero de dias }
  {acrescido da hora, minuto, segundo e valores. Note que o segundo valor ?   }
  {armazenado em um double, uma vez que pode ter uma parte fracion醨ia.        }
  {day        - Dias do m阺                                                    }
  {hour       - horas do dia                                                   }
  ...
  {Esta fun玢o converte o dia, a hora, minuto e segundos para o padr鉶 SPSS    }
  {day        - Dias do m阺                                                    }
  {hour       - horas do dia                                                   }
  {minute     - Minutos da hora                                                }
  {second     - Segundos do minuto                                             }
  ...
  {Esta fun玢o adiciona um simples atributo ao arquivo                         }
  {Se atributo j?existir ele ser?sobre escrito, para ele n鉶 sobre escrevido }
  {deve ser passado -1 no atribsub                                             }
  {handle       - Handle do arquivo.                                           }
  {varName      - Nome da vari醰el                                             }
  ...
  {Esta fun玢o adiciona uma multi resposta tipo string ao dicion醨io do arquivo}
  {handle       - Handle do arquivo.                                           }
  {mrSetName    - Nome da multipla resposta com tamanho m醲imo de 64 bytes long}
  {               que come鏴 com o sinalizador "$" e tenha um nome v醠ido      }
  {mrSetLabel   - Label(Pergunta) para a multi resposta com o tamanho m醲imo de}
  {               256 bytes long. Pode ser vazio ou null para indicar que n鉶 閩
  {               o label desejado                                             }
  {isDichotomy  - Se n鉶 null ser鉶 codificadas no dichotomies e zero de outra }
  {               forma                                                        }
  ...
  {               ser 1? characters long e ignorados caso contr醨io. Pode ser }
  {               nul se isDichotomy igual a zero                              }
  {varNames     - Array terminado em null contendo o nome das vari醰eis        }
  {               Todas as vari醰eis devem ter o nome curto(acho que tamanho   }
  {               m醲imo tem que ser 8).                                       }
  {numVars      - Numero de vari醰eis da lista (varNames).                     }
  ...
  {Esta fun玢o adiciona uma multi resposta a partir da estrututa no dicionario }
  {Ao adicionar na estrutura, definir o nome 鷑ico e as vari醰eis devem existir}
  {e ser do tipo adequado - num閞ica ou string dependendo do qIsNumeric.       }
  {handle       - Handle do arquivo.                                           }
  ...
  {Esta fun玢o adiciona uma multi resposta tipo numerica ao dicion醨io         }
  {handle       - Handle do arquivo.                                           }
  {mrSetName    - Nome da multipla resposta com tamanho m醲imo de 64 bytes long}
  {               que come鏴 com o sinalizador "$" e tenha um nome v醠ido      }
  {mrSetLabel   - Label(Pergunta) para a multi resposta com o tamanho m醲imo de}
  ...
  {Esta fun玢o ?similar a sspsaddfileatribute, mas que opera em uma 鷑ica     }
  {vari醰el da esrutura de atributos.                                          }
  {handle       - Handle do arquivo.                                           }
  {varName      - Nome da vari醰el                                             }
  {attribName   - Nome do atributo                                             }
  {attribSub    - Origem do atributo ou -1                                     }
  ...
  {Desaloca玢o de m閙oria}
  {28/04/2008}

  {Esta fun玢o desaloca a mem髍ia alocada dinamicamente pelas fun珲es          }
  ...
  {Esta fun玢o ?chamada p/ liberar a memoria alocada pela fun玢o              }
  ...
  {Esta fun玢o libera a memoria alocada pela fun玢o spssGetMultRespDefs        }
  TspssFreeMultRespDefs      = function(mrespDefs : PAnsiChar)  : integer; {$IFDEF WIN32} stdcall; {$ENDIF}

  {Esta fun玢o libera a memoria alocada pela fun玢o spssGetMultRespDefByIndex  }
  TspssFreeMultRespDefStruct = function(pSet : PspssMultRespDef) : integer; {$IFDEF WIN32} stdcall; {$ENDIF}

  {Esta fun玢o libera da memoria dois arrays PTValuec e numero de labels,      }
  {alocados pela fun玢o spssGetVarCValueLabels                                 }
  TspssFreeVarCValueLabels   = function(values : PTValuec; labels : PTValuec; numLabels : integer) : integer; {$IFDEF WIN32} stdcall; {$ENDIF}

  {Esta fun玢o libera a memoria alocada pela fun玢o spssGetVariableSets        }
  TspssFreeVariableSets      = function(varSets : PAnsiChar) : integer; {$IFDEF WIN32} stdcall; {$ENDIF}

  {Esta fun玢o libera da memoria os arrays PTValued e PTValuec e o numero de   }
  {labels alocados pela fun玢o spssGetVarNValueLabels                          }
  TspssFreeVarNValueLabels   = function(values : PTValued; labels : PTValuec; numLabels : integer) : integer; {$IFDEF WIN32} stdcall; {$ENDIF}

  {Esta fun玢o libera da memoria os arrays PTValuec e PTValuei e o numero de   }
  {variaveis alocados pela fun玢o spssGetVarNames                              }
  TspssFreeVarNames          = function(varNames : PTValuec; varTypes : PTValuei; numVars : integer) : integer; {$IFDEF WIN32} stdcall; {$ENDIF}


  {Pega informa珲es sobre o arquivo}
  {25/04/2008}

  {Esta fun玢o retorna o tamanho(size) do case associado ao arquivo            }
  ...
  {***** N鉶 entendi como funciona *****                                       }
  TspssGetCaseWeightVar      = function(const handle : integer; const varName : PAnsiChar) : integer; {$IFDEF WIN32} stdcall; {$ENDIF}

  {Esta fun玢o 1 se arquivo compactado e 0 caso nao seja compactado            }
  TspssGetCompression        = function(const handle : integer; var compSwitch : integer) : integer; {$IFDEF WIN32} stdcall; {$ENDIF}

  {Esta fun玢o reporta informa珲es sobre data TRENDS no arquivo                }
  {Ela aloca em tempo de execucao o array dateInfo com o tamanho numVars, ap髎 }
  {utiliza玢o do array ?necess醨io sua libera玢o da memoria feita pela fun玢o }
  ...
  {Fun玢o n鉶 necessariamente importarte apartir da vers鉶 6 do SPSS           }
  {Se o arquivo n鉶 tiver respostas a fun玢o ir?rentornar -1 em casecount, mas}
  ...
  {N鉶 pode ser usada se arquivo aberto spssOpenAppend                         }
  TspssGetEstimatedNofCases  = function(const handle : integer; Var caseCount : Longint) : integer; {$IFDEF WIN32} stdcall; {$ENDIF}

  {Esta fun玢o retorna todos os atributos do arquivo                           }
  {Fun玢o aloca a memoria necess醨io para retornar os nome e seu valores       }
  ...
  {Esta fun玢o provem numero da pagina(Handle) do windows relacionado a arquivo}
  TspssGetFileEncoding = function(const handle: integer; var pszEncoding: PAnsiChar):
      integer;

  {Esta fun玢o copia o label do arquivo para o buffer(PAnsiChar) id.               }
  ...
  {Esta fun玢o retorna a codifica玢o da pagina}
  TspssGetInterfaceEncoding  = function() : integer; {$IFDEF WIN32} stdcall; {$ENDIF}

  {Esta fun玢o retorna o numero de multi resposta do dicionario                }
  TspssGetMultRespCount      = function(const handle : integer; var nSets : integer) : integer; {$IFDEF WIN32} stdcall; {$ENDIF}

  {Esta fun玢o obtem os dados de uma multi resposta sentando ela na estrutura  }
  {Ap髎 utiliza玢o da fun玢o mem. deve ser libera c/ spssFreeMultRespDefStruct }
  ...
  {Pegas as informa珲es do case}

  {Esta fun玢o pega uma valor do timpo string do case corrente}
  ...
  {Esta fun玢o retorna o handle da variavel, que pode ser usado tanto para     }
  ...
  {Esta fun玢o reporta o valor do atributo de alinhamento da vari醰el}
  ...
  {Esta fun玢o indica se a variavel tipo string possui valor}
  ...
  {Esta fun玢o reporta o tamanho(width) de uma vari醰el}
  ...
  {Esta fun玢o reporta o nome curto da vari醰el}
  ...
  {Esta fun玢o permite ao usu醨io controlar o numero de caracteres ele deseja  }
  ...
  {Esta fun玢o reporta as informa珲es da variaveis do arquivo de dados         }
  TspssGetVariableSets       = function(const handle : integer; varNames : PTValuec) : integer; {$IFDEF WIN32} stdcall; {$ENDIF}

  {Esta fun玢o pega nome e o tipo de uma variavel presente no arquivo          }
  ...
  {Esta fun玢o copia o label da vari醰el passada para fun玢o para ponteiro     }
  ...
  {Esta fun玢o retorna o label associado a vari醰el trazendo apenas o numero de}
  {bytes informado                                                             }
  ...
  {Esta fun玢o retorna o valor medio do level atribuido a variavel             }
  ...
  {Esta fun玢o ?uma combina玢o para busca de valores de variaveis             }
  ...
  {Esta fun玢o retorna valor de uma label que corresponde a um valor especifico}
  ...
  {Esta fun玢o retorna formata玢o(PrintType, Decimal e tamanho) de uma variavel}
  ...
  {Esta fun玢o escreve a formata玢o de uma variavel no arquivo                 }
  ...
  {Esta fun玢o determina a se a codifica玢o do arquivo ?compativel            }
  ...
  {Esta fun玢o reescreve os atributos de uma vari醰el}
  ...
  {Esta fun玢o seta o falta de valor de uma vari醰el string}
  ...
  {Esta seta o tamanho(width) de uma coluna de uma vari醰el}
  ...
  {Esta fun玢o altera ou adiciona a um label de uma vari醰el string}
  ...
  {Esta fun玢o altera ou adiciona um ou mais labels de uma ou mais vari醰eis string}
  ...
  {Esta fun玢o seta o label de uma vari醰el}
  ...
  {Esta fun玢o seta o valor medio de uma level do atributo da vari醰el}
  ...
  {Esta fun玢o seta a falta de valor de uma vari醰el numeric}
  ...
  {Esta fun玢o altera ou adiciona a um label de uma vari醰el numeric}
  ...
  {Esta fun玢o altera ou adiciona um ou mais labels de uma ou mais vari醰eis numeric}
  ...
  {Esta fun玢o cria uma nova vari醰el string ou numeric no arquivo}
  ...
  {Esta fun玢o seta as formata玢o de impress鉶 da vari醰el}
  ...
  {Esta fun玢o seta as formata玢o de escrita da vari醰el}
...
initialization
  if not LoadLib() then begin
    ShowMessage('N鉶 foi possivel carregar SPSSIO32.DLL');
    halt(0);
  end;
